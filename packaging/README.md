...
- `mainspring.spec` -- PyInstaller build spec, driven by `tools/build_exe.ps1` (never invoke
  `pyinstaller` on this file directly: the script sets the dist/build paths and the
  onefile/onedir mode). `entrypoint.py` is its entry script, since PyInstaller freezes a file,
  not the `pyproject.toml` console-script name.
